...
class AdaptiveQuad(bpy.types.Node, AnimationNode): 
    ...
    def drawAdvanced(self, layout):
        ...
        layout.label("CustomBaseSide or default 2x2 plane")
        layout.operator("wm.call_menu", text = "Info / update! v0.3", icon = "INFO").name = "an.show_help_adaptive_quad"

    def updateSocketVisibility(self):
        if self.useBbox: 
            self.useCustomBaseSide = False
        self.inputs["Base Side"].hide = not self.useCustomBaseSide


    def execute(self, polygon, polygonIndices, vertices, vertexLocations, zFactor, baseSide): #, matrix
        
        VL = vertexLocations    #the instance mesh
        ...
        if all((len(polygon.vertices) > 2, len(polygonIndices) > 2, len(vertices) > max(i for i in polygonIndices))):
            ...
            n3 = vertices[polygonIndices[-1]].normal  #escaping non quads, take last, + tris 2=3
            nP = polygon.normal
        else:   
            #if there is no poly, fall back to default 2x2 plane etc
            q0 = Vector((-bs, -bs, 0))
            q1 = Vector(( bs, -bs, 0))
            q2 = Vector(( bs,  bs, 0))
            q3 = Vector((-bs,  bs, 0))
        ...
        for v in VL:
            ...
            DeformedLocations.append(qdef+ndef*v[2]*zFactor)  #matrix*
