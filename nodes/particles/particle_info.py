...
#        if any ([ isLinked["hairTime"], isLinked["hairWeight"], isLinked["hairPoints"], isLinked["hairPointsLocal"] ]):
#            lines.append("    if particle.hair_keys:")

#            if isLinked["hairTime"]:        lines.append(" " * 8 + "hairTime = [key.time for key in particle.hair_keys:]")
#            if isLinked["hairWeight"]:      lines.append(" " * 8 + "hairWeight = [key.weight for key in particle.hair_keys:]")
#            if isLinked["hairPoints"]:      lines.append(" " * 8 + "hairPoints = [key.co for key in particle.hair_keys:]")
#            if isLinked["hairPointsLocal"]: lines.append(" " * 8 + "hairPointsLocal = [key.co_local for key in particle.hair_keys:]")
